...
export function NewShareModal({ open, onOpenChange, onShareSuccess }: NewShareModalProps) {
  ...
  const vaultEntries = vault?.Vault ? [
    ...(vault.Vault.entries?.login || []).map(e => ({ id: e.id, name: e.entry_name, type: e.type })),
    ...(vault.Vault.entries?.card || []).map(e => ({ id: e.id, name: e.entry_name, type: e.type })),
    ...(vault.Vault.entries?.note || []).map(e => ({ id: e.id, name: e.entry_name, type: e.type })),
    ...(vault.Vault.entries?.sshkey || []).map(e => ({ id: e.id, name: e.entry_name, type: e.type })),
    ...(vault.Vault.entries?.identity || []).map(e => ({ id: e.id, name: e.entry_name, type: e.type })),
  ] : [];
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const newSharedEntry: SharedEntry = {
        id: response.id || `shared-${Date.now()}`,
        entry_name: selectedVaultEntry?.name || "Shared Entry",
        entry_type: (selectedVaultEntry?.type || "note") as any,
        status: "active",
              ...
              <SelectContent>
                {vaultEntries.map((entry) => (
                  <SelectItem key={entry.id} value={entry.id}>
                    <span className="capitalize">{entry.type}</span> • {entry.name}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
