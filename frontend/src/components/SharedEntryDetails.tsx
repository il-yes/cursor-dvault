...
export function SharedEntryDetails({ entry, view, updateRecipients }: SharedEntryDetailsProps) {
	...
	const renderSensitiveField = (
		...
		const effectiveEntry = full || (decryptedEntry as any) || entry;

		const renderEntryContent = () => {
			const type = (
				effectiveEntry?.type ||
				effectiveEntry?.entry_type ||
				effectiveEntry?.record_type ||
				""
			).toLowerCase();
			...
			switch (type) {
				case "login": {
					...
					const attachments = full?.attachments || full?.attachements || login.attachments;
					...
					return (
						<div className="space-y-3">
							...
							{attachments?.length > 0 && <RenderAttachements entry={{ attachments }} />}
						</div>
					);
				}
				...
				case "note": {
					...
					const attachments = full?.attachments ?? note.attachments ?? [];
					const text = (note as any).note ?? (note as any).content ?? (note as any).additionnal_note ?? (note as any).additional_note;
					console.log("full attachments length with attachments : ", attachments.length > 0);
