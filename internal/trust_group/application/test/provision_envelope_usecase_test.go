...
func TestProvisionTrustGroupDeviceEnvelope_Success(t *testing.T) {
	...
	provisionUC := trustgroup_usecases.NewProvisionTrustGroupMemberEnvelopeUseCase(repo, nil, addEnvUC, keyringSvc)
	...
	kr := &vaults_domain.VaultKeyring{UserID: "vault_alice", VaultID: "vault-alice-v"}
	testKEK := make([]byte, 32)
	for i := range testKEK {
		testKEK[i] = byte(i + 1)
	}
	_, err = keyringSvc.StoreTrustGroupKEK(kr, tg.ID, tg.KEKVersion, testKEK)
	require.NoError(t, err)
	...
	updatedTg, err := provisionUC.Execute(ctx, req, kr)
	require.NoError(t, err)
