...
export function EntryDetailPanel({ entry, editMode, onEdit, onSave, onCancel, onShare, onDelete }: EntryDetailPanelProps) {
  ...
  };// In useEffect: initialize editData with all editable fields
  useEffect(() => {
    if (entry && editMode) {
      const initialData: Partial<VaultEntry> = { entry_name: entry.entry_name };

      Object.keys(entry).forEach(key => {
        if (key !== "id" && key !== "created_at" && key !== "updated_at" && key !== "type" && key !== "trashed" && key !== "is_draft") {
          // @ts-ignore
          initialData[key] = entry[key];
        }
      });
  ...
  // Render editable input dynamically
  const renderEditableField = (fieldName: string, label: string, isSensitive: boolean = false) => {
    const value = editMode ? (editData[fieldName] ?? "") : (entry ? entry[fieldName] ?? "" : "");
    ...
    if (!editMode) {
      // Display mode
      if (isSensitive) return renderSensitiveField(fieldName, label, true);
      return (
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="space-y-4">
          {Object.keys(entry).map((key) => {
            if (["id", "created_at", "updated_at", "trashed", "is_draft", "type"].includes(key)) return null;

            const label = key.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase());
            const isSensitive = ["password", "number", "cvc", "private_key"].includes(key);

            return <div key={key}>{renderEditableField(key, label, isSensitive)}</div>;
          })}
