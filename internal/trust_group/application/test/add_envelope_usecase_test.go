...
func TestAddTrustGroupKeyEnvelope_Success(t *testing.T) {
	...
	uc := trustgroup_usecases.NewAddTrustGroupKeyEnvelopeUseCase(repo)
...
func TestAddTrustGroupKeyEnvelope_Failures(t *testing.T) {
	...
	t.Run("Duplicate active envelope for member", func(t *testing.T) {
		// First addition
		_, err := uc.Execute(ctx, trustgroup_dtos.AddTrustGroupKeyEnvelopeRequest{
			TrustGroupID: tg.ID,
			MemberID:     "member-vault-1",
		...
		// Second addition for same member and version
		_, err = uc.Execute(ctx, trustgroup_dtos.AddTrustGroupKeyEnvelopeRequest{
			TrustGroupID: tg.ID,
			MemberID:     "member-vault-1",
			KEKVersion:   1,
