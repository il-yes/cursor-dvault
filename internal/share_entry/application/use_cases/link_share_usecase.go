...
type AnkhoraClientInterface interface {
	CreateLinkShare(ctx context.Context, req share_entry_application_dto.LinkShareCreateRequest) (*tracecore.CreateLinkShareResponse, error)
...
func NewLinkShareUseCase(repo share_entry_domain.Repository, tc AnkhoraClientInterface, d share_entry_application_events.EventDispatcher, crypto ClientCryptoService) *LinkShareUseCase {
...
func (uc *LinkShareUseCase) CreateLinkShare(ctx context.Context, email string, req share_entry_application_dto.LinkShareCreateRequest) (*share_entry_domain.LinkShare, error) {
	// ----------------------------------------------------------
	...
	// 3. Create Link share
	// ----------------------------------------------------------
	share := share_entry_domain.LinkShare{
		Payload: req.Payload,
		...
		Metadata: share_entry_domain.Metadata{
...
func (uc *LinkShareUseCase) ListLinkSharesByMe(ctx context.Context, email string) (*[]tracecore.WailsLinkShare, error) {
	if uc.tc == nil {
		utils.LogPretty("share_entry_use_case - ListLinkSharesByMe - tc is nil", nil)
	...
	if err != nil {
		utils.LogPretty("share_entry_use_case - ListLinkSharesByMe - tc.ListLinkSharesByMe error: %v\n", err)
		return nil, err
	}
